...
git clone --depth=1 https://github.com/khahmed94/odoo-18-docker-compose $DESTINATION
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  # macOS sed syntax
  sed -i '' 's/10018/'$PORT'/g' $DESTINATION/docker-compose19.yml
  sed -i '' 's/20018/'$CHAT'/g' $DESTINATION/docker-compose19.yml
else
  # Linux sed syntax
  sed -i 's/10018/'$PORT'/g' $DESTINATION/docker-compose19.yml
  sed -i 's/20018/'$CHAT'/g' $DESTINATION/docker-compose19.yml
...
# Run Odoo
docker-compose -f $DESTINATION/docker-compose19.yml up -d
